...
const createDoetList = async (req: express.Request, res: express.Response) => {
    try {
        logger.info(`Recieved request to create doetList: ${JSON.stringify(req.body)} for user: ${req.params.user_id}`);
        ...
        if (user === null) {
            ...
            const newDoetList = await new DoetList({user: user._id, ...req.body}).save();
            user.doetLists.push(newDoetList._id);
            await user.save();

            respond.send200(`Successfully created new doetList: ${newDoetList._id}`, newDoetList, res);
        }
    }
    catch (err) {
...
const getDoetListById = async (req: express.Request, res: express.Response) => {
    try {
        ...
        if (doetList === null) {
            ...
            respond.send200(`Successfully found doetList: ${doetList._id}`, doetList, res);
        }
    }
    catch (err) {
...
const updateDoetList = async (req: express.Request, res: express.Response) => {
    try {
        ...
        const doetList = await DoetList.findByIdAndUpdate(req.params.id, {$set: req.body}, {new: true, runValidators: true});

        if (doetList === null) {
            respond.send404(`Failed to get doetList: ${req.params.id}`, res);
        }
        else {
            respond.send200(`Successfully updated doetList: ${doetList._id}`, doetList, res);
        }
    }
    catch (err) {
...
const deleteDoetList = async (req: express.Request, res: express.Response) => {
    try {
        ...
        if (doetList === null) {
            ...
            respond.send200(`Successfully deleted doetList: ${doetList._id}`, doetList, res);
        }
    }
    catch (err) {
