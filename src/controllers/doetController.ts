...
const createDoet = async (req: express.Request, res: express.Response) => {
    try {
        logger.info(`Recieved request to create new doet: ${JSON.stringify(req.body)}`);
        const doetList = await DoetList.findById(req.params.doetList_id);

        if (doetList === null) {
            respond.send404(`Failed to find doetList: ${req.params.doetList_id}`, res);
        }
        else {
            const newDoet = await new Doet({doetList: doetList._id, ...req.body}).save();
            ...
            if (newDoet === null) {
                ...
                respond.send200(`Successfully created new doet: ${newDoet._id}`, newDoet, res);
            }
        }
    }
    catch (err) {
...
const updateDoetById = async (req: express.Request, res: express.Response) => {
    try {
        ...
        const doet = await Doet.findByIdAndUpdate(req.params.id, {$set: req.body}, {new: true, runValidators: true});
